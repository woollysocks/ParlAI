...
class OnboardingWorld(MTurkOnboardWorld):
    ...
    def parley(self):
        ...
        ad['id'] = 'System'
        ad['text'] = (
            "Welcome onboard! Before starting the HIT, you need to complete the onboarding.\n\nPlease read the instructions carefully and evaluate the following example by deciding if the claim fits the label and the prompt. Thank you!"
        )
...
class MultiRoleAgentWorld(MTurkTaskWorld):
    ...
    def __init__(self, opt, task, mturk_agents):
        ...
        self.ranker_bonus = 0.3
        self.hit_bonus = {self.agents[0]:0, self.agents[1]:0}
        ...
        if self.meta_turn < self.max_meta_turns:
            if self.turns == 0:
                ...
                prompt = {
                        'id': 'Prompt',
                        'text': premise,
                        'task_data': {'writing':True, 'feedback':False, 'ranking':False}
                    }
            ...
            if self.turns == 2:
                ...
                for i in range(self.num_agents):
                    self.entailments[i] = {'id':'Claim '+str((i%2)+1), 'text': self.hypotheses_collect[i]['text'], 'task_data': {'respond_with_form':[], 'writing':False, 'onboard':False, 'ranking':True}}
                ...
                for agent in self.agents:
                    agent.observe({'id':'Phase 2', 
                                   'text':'<b>Thank you for writing your claims! Now please read the other person\'s claim, compare yours with theirs, and provide a ranking</b>'})
                time.sleep(3)
                ...
                for agent in self.evaluators_ent:
                    ...
                    if evaluation is not None:
                        ...
                        if agent not in self.evaluators_cont:
                            ...
                            this_claim = int(agent.demo_role[-1])-1
                            mappy[agent], _ = self.observe_hypotheses(prompt, label, [agent], show_hypotheses[this_claim], show_hypotheses[abs(this_claim-1)])
            ...
            if self.turns == 4:
                ...
                for j in self.sets.keys():
                    ...
                    for i, rankings in enumerate(set_evals[j]):
                        ...
                        writer_bonus_message = {'id': label, 'text': 'You agreed with the other person and ranked higher! Bonus = $' + str(self.writer_bonus*2) +'.'}
                        writer_rank2_message = {'id': label, 'text': 'Unfortunately, you ranked 2nd.'}
                        
                        noagrm_nobonus_message = {'id': label, 'text': 'Unfortunately you didn\'t agree on the ranking.'} # Set as ranker_bonus to avoid incentivizing workers to always disagree on ranking
                        writer_incomplete_message = {'id': label, 'text': 'Unfortunately both of you timed out and did not submit rankings.'}
                        
                        agrm_bonus_message = {'id': label, 'text': 'You agreed with the other person! Bonus = $' + str(self.ranker_bonus) + '.'}
                        # noagrm_nobonus_message = {'id': label, 'text': 'You disagreed on the ranking.'}
                        eval_you_incomplete = {'id': label, 'text': 'Unfortunately you timed out and did not submit a ranking.'}
                        eval_other_incomplete = {'id': label, 'text': 'Unfortunately the other person timed out and did not submit a ranking.'}
                        ...
                        if rankings[0]['text'] != rankings[1]['text']:
                            if rankings[0]['text'] is None:
                                ...
                                if rankings[0]['text'] == 'Your claim' and rankings[0]['id'] == 'Person 1':
                                    evaluator_feedback.append((evaluators[0], writer_bonus_message))
                                    evaluator_feedback.append((evaluators[1], agrm_bonus_message))
                                    worker_bonuses[evaluators[0]] += self.writer_bonus
                                elif rankings[0]['text'] == 'Their claim' and rankings[0]['id'] == 'Person 1':
                                    evaluator_feedback.append((evaluators[1], writer_bonus_message))
                                    evaluator_feedback.append((evaluators[0], agrm_bonus_message))
                                    worker_bonuses[evaluators[1]] += self.writer_bonus
                                # elif rankings[0]['text'] == 'Their Claim' and rankings[0]['id'] == 'Person 2':
                                #     evaluator_feedback.append((evaluators[0], writer_bonus_message))
                                #     worker_bonuses[evaluators[0]] += self.writer_bonus
                                #     evaluator_feedback.append((evaluators[1], agrm_bonus_message))
                                # elif rankings[0]['text'] == 'Your Claim' and rankings[0]['id'] == 'Person 2':
                                #     evaluator_feedback.append((evaluators[1], writer_bonus_message))
                                #     evaluator_feedback.append((evaluators[0], agrm_bonus_message))
                                #     worker_bonuses[evaluators[1]] += self.writer_bonus
                                else:
                            ...
                            if rankings[0]['text'] is None:
                                for evaluator in evaluators:
                                    evaluator_feedback.append((evaluator, writer_incomplete_message))
                            else:
                                for evaluator in evaluators:
                            ...
                            #     writer_feedback.append((writer, noagrm_nobonus_message))
                        ...
                        if eval0_explain != '':
                            evaluator_feedback.append((evaluators[1], {'id':'Explanation from other person', 'text': eval0_explain}))
                            for writer in writers:
                        ...
                        for n, writer in enumerate(writers):
                            evaluator_feedback.append((writers[n], {'id':'Your claim', 'text':both_hypotheses[n]}))
                            evaluator_feedback.append((writers[n], {'id':'Other claim', 'text':both_hypotheses[(n+1)%2]}))
                ...
                for agent in self.agents:
                    agent.observe({"id":"Your total bonus for this round is", 
                                  "text":"$"+str(worker_bonuses[agent])+" (the bonuses are sent when the HIT is submitted)"})
                    self.hit_bonus[agent] += worker_bonuses[agent]
                    # agent.pay_bonus(worker_bonuses[agent], "For work on the 2-person textual entailment game.")
                ...
                for i, evals in enumerate(self.evals):
                    ...
                    for j in range(len(evals)):
                        ...
                        if not skip:
                            ...
                            this_data['assignment_id'] = agent.assignment_id
    ...
    def give_feedback(self, all_feedback, agents, writing=False):
        for i in all_feedback.keys():
            ...
            if not writing: # evaluation feedback
                ...
                agents[i].observe({'id':'Thank you! Here are your bonuses', 'text':text, 'task_data': {'respond_with_form':[], 'feedback':True}})
    ...
    def review_work(self):
        ...
        for agent in self.agents:
            agent.approve_work()
            agent.pay_bonus(round(self.hit_bonus[agent],2), "For work on the 2-person textual entailment game.")
        pass
